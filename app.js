...
function reset(){ state.reset(); state.language='en-IN'; state.seminar=false; }
...
function maya(){
  ...
  if(state.stage==='evidence')m=state.investigations.length<2?'Good. Keep collecting evidence before committing to a cause. What else can you verify?':'Now compare the evidence. Which component is actually showing abnormal behavior?';
  ...
  if(state.stage==='learning'||state.stage==='results')m='Now look at what you did well and where your engineering reasoning can become stronger.';
...
function feedback(){
  ...
  const firewallChecked=state.investigations.includes('firewall');
  const webChecked=state.investigations.includes('web');
  const dbChecked=state.investigations.includes('db');
  ...
  if(diagnosisCorrect){
    items.push(`<div class="feedback good"><strong>✓ Diagnosis was correct.</strong><span>You identified the firewall as the component blocking the user's HTTPS request.</span></div>`);
  }else{
    items.push(`<div class="feedback improve"><strong>✗ Diagnosis: ${esc(state.hypothesis||'not selected')}</strong><span>The evidence showed DNS was resolving, the web server was running, and the database was healthy. The abnormal evidence was the disabled <code>ALLOW-HTTPS-PROD</code> rule blocking TCP/443.</span></div>`);
  }

  if(actionCorrect){
    items.push(`<div class="feedback good"><strong>✓ Remediation was correct.</strong><span>Re-enabling <code>ALLOW-HTTPS-PROD</code> addresses the actual failure: incoming HTTPS traffic was being blocked.</span></div>`);
  }else{
    items.push(`<div class="feedback improve"><strong>✗ Remediation would not fix the incident.</strong><span>${state.action?`You chose “${esc(state.action)}”.`: 'No remediation was selected.'} That does not address the disabled firewall rule blocking TCP/443. The appropriate action is to re-enable <code>ALLOW-HTTPS-PROD</code>, then verify external HTTPS access.</span></div>`);
  }

  if(firewallChecked){
    items.push(`<div class="feedback good"><strong>✓ You found the key evidence.</strong><span>The firewall audit showed a 09:10 configuration change, the HTTPS rule disabled, and incoming TCP/443 blocked. This is the strongest evidence in the incident.</span></div>`);
  }else{
    items.push(`<div class="feedback improve"><strong>→ Missing the decisive evidence.</strong><span>You did not inspect the firewall. The decisive clue was the disabled <code>ALLOW-HTTPS-PROD</code> rule blocking incoming TCP/443.</span></div>`);
  }

  if(!webChecked || !dbChecked){
    const missing=[]; if(!webChecked)missing.push('web server'); if(!dbChecked)missing.push('database');
    items.push(`<div class="feedback tip"><strong>→ Strengthen your investigation.</strong><span>You could have checked the ${missing.join(' and ')} to rule out other plausible causes before deciding. A strong engineer compares alternatives rather than jumping to the first explanation.</span></div>`);
  }

  if(reasoningSignals.length>=4){
    items.push(`<div class="feedback good"><strong>✓ Your explanation contained useful technical evidence.</strong><span>You connected several relevant signals from the incident. Keep making the evidence → cause → action chain explicit.</span></div>`);
  }else{
    items.push(`<div class="feedback improve"><strong>→ Your reasoning needs more evidence-to-cause connection.</strong><span>Explain <em>what you observed</em>, <em>why it points to the root cause</em>, and <em>why your chosen action fixes that cause</em>. For example: “The firewall rule was disabled at 09:10, so incoming HTTPS traffic on TCP/443 was blocked. The web server and database were healthy, so re-enabling the rule should restore access.”</span></div>`);
  }

  if(state.hints>0){
    items.push(`<div class="feedback tip"><strong>💡 You used ${state.hints} Maya hint${state.hints>1?'s':''}.</strong><span>Hints are available when you are stuck, but try to gather and compare evidence independently before asking Maya next time.</span></div>`);
  }
...
function render(){
  ...
  if(state.stage==='evidence')return shell(`<div class="eyebrow">Step 2 · Gather evidence</div><h2>What did you find?</h2><p class="sub">Review the result, then choose another investigation. Build the picture before you diagnose.</p>${state.investigations.map(id=>`<div class="evidence"><h3>${esc(scenario.evidence[id][0])}</h3><pre>${esc(scenario.evidence[id][1])}</pre></div>`).join('')}<div class="actions">${scenario.investigation.filter(x=>!state.investigations.includes(x[0])).map(x=>`<button class="choice" data-check="${x[0]}"><strong>${esc(x[1])}</strong><span>${esc(x[2])}</span></button>`).join('')}<button class="primary" data-diagnose="1">I’m ready to form a hypothesis</button>${back('Back to investigation','investigate')}</div>`);
  if(state.stage==='diagnose')return shell(`<div class="eyebrow">Step 3 · Diagnose</div><h2>What is the most likely cause?</h2><p class="sub">Choose the explanation that best fits the evidence you collected.</p><div class="actions">${scenario.hypotheses.map(x=>`<button class="choice" data-hypothesis="${x.id}"><strong>${esc(x.label)}</strong></button>`).join('')}${back('Back to evidence','evidence')}</div>`);
  if(state.stage==='decision'||state.stage==='action')return shell(`<div class="eyebrow">Step 4 · Restore</div><h2>What would you do?</h2><p class="sub">Select the action that should restore service.</p><div class="actions">${scenario.actions.map(x=>`<button class="choice" data-fix="${x.id}"><strong>${esc(x.label)}</strong></button>`).join('')}${back('Back to diagnosis','diagnose')}</div>`);
