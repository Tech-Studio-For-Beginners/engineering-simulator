export function scoreRun(scenario,state){
 const investigation=25*Math.min(state.investigations.length/3,1);
 const evidence=20*Math.min(state.investigations.filter(id=>["web","db","firewall"].includes(id)).length/3,1);
